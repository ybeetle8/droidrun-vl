...
OLLAMA_INSTALL_DIR="/qwen/qwen"
BINDIR="$OLLAMA_INSTALL_DIR/bin"

# Ensure BINDIR is in PATH
if ! echo $PATH | grep -q "$BINDIR"; then
    warning "Warning: $BINDIR is not in PATH. You may need to add it manually."
fi
...
configure_systemd() {
    if ! id ollama >/dev/null 2>&1; then
        ...
        $SUDO useradd -r -s /bin/false -U -m -d /qwen/qwen/share/ollama ollama
    fi
